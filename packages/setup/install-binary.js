...
const RELEASES_API = 'https://api.github.com/repos/lacs-foundation/sysknife/releases/latest';
...
function detectPlatform() {
  if (process.platform !== 'linux') {
    throw new Error(
      `SysKnife is Linux-only; see https://github.com/lacs-foundation/sysknife for cross-distro plans.\n` +
...
async function installBinaryIfMissing(opts) {
  ...
  try {
    ...
    step('https://github.com/lacs-foundation/sysknife/releases/latest');
    process.exit(1);
  }
  ...
  if (!cliUrl || !daemonUrl || !sumsUrl) {
    ...
    step(daemonAsset);
    step(sumsAsset);
    err('Install manually from: https://github.com/lacs-foundation/sysknife/releases/latest');
    process.exit(1);
  }
