...
//!
//! The default log path is `$XDG_DATA_HOME/lacs/safety-audit.jsonl`
//! (falling back to `~/.local/share/lacs/safety-audit.jsonl`).
...
impl SafetyAuditLog {
    ...
    ///
    /// Returns `$XDG_DATA_HOME/lacs/safety-audit.jsonl` if set,
    /// otherwise `~/.local/share/lacs/safety-audit.jsonl`.
    pub fn default_path() -> PathBuf {
        if let Ok(xdg) = std::env::var("XDG_DATA_HOME") {
            if !xdg.is_empty() {
                return PathBuf::from(xdg).join("lacs").join("safety-audit.jsonl");
            }
        }
...
mod tests {
    ...
    fn default_path_respects_xdg_data_home() {
        ...
        assert!(
            path.to_str().unwrap().ends_with("lacs/safety-audit.jsonl"),
            "default path should end with lacs/safety-audit.jsonl, got: {}",
            path.display()
        );
    }
}
