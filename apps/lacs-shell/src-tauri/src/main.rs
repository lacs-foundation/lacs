    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            if connected && !was_connected {
                let _ = app.emit("lacs:daemon-status", json!({"status": "connected"}));
                backoff = Duration::from_secs(1); // reset for next outage
                was_connected = true;
            } else if !connected && was_connected {
                let _ = app.emit("lacs:daemon-status", json!({"status": "reconnecting"}));
                was_connected = false;
            }
...
fn main() {
    ...
    tauri::Builder::default()
        .setup(|app| {
            ...
            // Emits `lacs:daemon-status` events with payload {"status": "connected"}
