...
//! `lacs "check disk usage"` and `lacs check disk usage` both produce an
...
pub enum Command {
    ...
    /// `lacs "check disk usage"` and `lacs check disk usage` both work.
...
mod tests {
    ...
    fn no_args_parses_with_no_command_and_no_flags() {
        let cli = Cli::try_parse_from(["lacs"]).unwrap();
    ...
    fn multi_word_intent_captured_as_intent_variant() {
        let cli = Cli::try_parse_from(["lacs", "check", "disk", "usage"]).unwrap();
    ...
    #[test]
    fn yes_flag_parsed() {
        let cli = Cli::try_parse_from(["lacs", "--yes", "intent"]).unwrap();
        assert!(cli.yes);
    }
    ...
    fn max_risk_low_medium_high_parsed() {
        for (input, expected) in [
            ...
            let cli = Cli::try_parse_from(["lacs", "--max-risk", input]).unwrap();
    ...
    fn dry_run_and_non_interactive_parsed() {
        let cli =
            Cli::try_parse_from(["lacs", "--dry-run", "--non-interactive", "intent"]).unwrap();
        assert!(cli.dry_run);
        assert!(cli.non_interactive);
    }
    ...
    fn json_and_timeout_and_step_by_step_parsed() {
        let cli = Cli::try_parse_from([
            "lacs", "--json", "--timeout", "30", "--step-by-step", "intent",
        ])
        .unwrap();
        assert!(cli.json);
    ...
    fn log_to_path_parsed() {
        let cli = Cli::try_parse_from(["lacs", "--log-to", "/tmp/lacs.log", "intent"]).unwrap();
        assert_eq!(cli.log_to.as_deref(), Some(std::path::Path::new("/tmp/lacs.log")));
    }
    ...
    fn history_subcommand_default_limit() {
        let cli = Cli::try_parse_from(["lacs", "history"]).unwrap();
        match cli.command {
    ...
    fn global_flags_work_before_subcommand() {
        let cli = Cli::try_parse_from(["lacs", "--yes", "--dry-run", "doctor"]).unwrap();
        assert!(cli.yes);
        assert!(cli.dry_run);
        assert!(matches!(cli.command, Some(Command::Doctor)));
    }
