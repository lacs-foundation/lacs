//! Human-readable output for the `lacs` CLI.
//!
...
use lacs_types::{JobState, ResultEnvelope, PreviewEnvelope};
...
/// Print a successful `lacs doctor` report via `log`.
...
/// Print a `lacs doctor` failure to stderr.
